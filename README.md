...
~~Как дела обстоят с Wayland не знаю, нужно ресёрчить (либо не разворачивать 
клиент 1С в Docker, а подключаться с хоста)~~.

С XWayland всё запускается и работает корректно.
...
- При дальнейших запусках использовать скрипт [run_services.sh](run_services.sh) `./run_services.sh` или управлять контейнерами вручную с помощью API Docker Compose. 
...
### Каталоги с лицензиями
- Сервер: `/var/1C/licenses` aka `/1c_licenses`
- Клиент: `/home/usr1cv8/.1cv8/1C/1cv8/conf`

### Миграция с предыдущих версий релизов
**Важно!** Никаких гарантий обратной совместимости.
<details>
  <summary>С версии 0.0.1-initial</summary>
  
  Только ручная. Если хотите сохранить лицензии, то скопируйте их из соответствующих директорий старых контейнеров 
  и перенесите в новые с аналогичными правами. Так же изменились некоторые директории монтирования и симлинки. 
  
  Подробнее можно посмотреть [здесь](https://github.com/alexandermyasnikov123/onec-community-docker/commit/e7d352f7dbe51a1d3535223b4390cf8a48d763fb).
</details>
